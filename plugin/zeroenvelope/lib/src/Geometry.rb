...
class Geometry
  ...
  def self.clipper(subject, clipping, operation)
    ...
    # merge neighbour vertices
    ohter_xs, ohter_ys = [], []
    subject, clipping = [subject, clipping].map do |polygons|
      polygons.map do |polygon| polygon.map do |point|
        x = nil
        ohter_xs.each do |other_x|
          next unless (other_x - point.first).abs < 1e-3
          x = other_x
          break
        end
        ...
        y = nil
        ohter_ys.each do |ohter_y|
          next unless (ohter_y - point.last).abs < 1e-3
          y = ohter_y
          break
        end
